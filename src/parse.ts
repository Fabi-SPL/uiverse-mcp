...
export function buildPreviewUrl(category: Category, id: string): string {
...
export function parseFilename(filename: string): { id: string; author: string; name: string } | null {
...
export function detectTailwind(html: string): boolean {
  if (extractCss(html)) return false;
