...
import { ensureCache, getCachePath, getCurrentSha, readMetadata, refreshCache, writeMetadata, } from "./cache.js";
import { buildIndex, findByCategoryId } from "./index-build.js";
...
const tools = [
    {
        ...
        description: "List all Uiverse component categories with their component counts. Categories: Buttons, Cards, Checkboxes, Forms, Inputs, Notifications, Patterns, Radio-buttons, Toggle-switches, Tooltips, loaders. Call this first when you don't know what's available.",
        inputSchema: {
            type: "object",
            properties: {},
    ...
    },
    {
        ...
        description: "List components in a specific category with pagination. Returns summaries only (id, author, name, tags, preview_url) — no HTML payload. Call get_component to fetch actual HTML/CSS for a specific component. Optional: filter by author.",
        inputSchema: {
            type: "object",
            properties: {
                category: {
    ...
    },
    {
        ...
        description: "Fuzzy search components by name, tags, or author. Returns top matches as summaries (no HTML). Use this when the user asks for a specific style like 'glowing button', 'glass card', 'ripple loader'. Optional category filter narrows the search.",
        inputSchema: {
            type: "object",
            properties: {
                query: {
                    type: "string",
                    description: "Search query — matches name, tags, and author. Examples: 'glass', 'neumorphic', 'ripple', 'glow'.",
                },
                category: {
                    type: "string",
                    enum: CATEGORIES,
    ...
    {
        name: "get_component",
        description: "Fetch the full HTML + CSS of a specific component by category + id. Returns markup, CSS (or note if Tailwind-based), tags, author, preview URL. Use after browsing with list_components or search_components.",
        inputSchema: {
            type: "object",
            properties: {
                category: {
                ...
                id: {
                    type: "string",
                    description: "Component id (filename without .html). Get this from list_components or search_components results.",
                },
            },
    ...
    },
    {
        ...
        description: "Return the public uiverse.io URL for visual preview of a component. Open this in a browser to see the component rendered with animations. Does NOT fetch HTML — use get_component for code.",
        inputSchema: {
            type: "object",
            properties: {
                category: {
    ...
    {
        name: "refresh_cache",
        description: "Pull the latest commits from the uiverse-io/galaxy repo and rebuild the in-memory index. Use when you suspect the cache is stale (last sync > 1 week). Returns commit SHA delta and new total count.",
        inputSchema: {
            type: "object",
            properties: {},
            additionalProperties: false,
    ...
    {
        name: "cache_status",
        description: "Report cache metadata: last sync time, current commit SHA, total components indexed, per-category counts. Use to debug sync issues.",
        inputSchema: {
            type: "object",
            properties: {},
            additionalProperties: false,
...
async function handleListComponents(input) {
...
async function handleGetComponent(input) {
    const summary = findByCategoryId(index, input.category, input.id);
    if (!summary) {
        throw new Error(`Component not found: category=${input.category}, id=${input.id}. Use list_components or search_components first.`);
    }
...
async function handleGetPreview(input) {
    const summary = findByCategoryId(index, input.category, input.id);
    if (!summary) {
        throw new Error(`Component not found: category=${input.category}, id=${input.id}.`);
    }
    return {
        category: input.category,
        id: input.id,
...
async function main() {
    ...
    process.stderr.write(`[uiverse-mcp] Ready. ${index.total} components across ${index.stats.length} categories. Cache: ${cachePath}\n`);
}
main().catch((err) => {
